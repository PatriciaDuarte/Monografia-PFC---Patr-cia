...
\begin{equation}
    ...
    \end{array} \right \}
\end{equation}
para $x = 0,1,2,\ldots,M-1$ e $y = 0,1,2,\ldots,N-1$. Observe que todas as quantidades necessárias para avaliar a Equação~\ref{eq:var_inter} são obtidas usando apenas o histograma de $f(x,y)$. Além do limiar ideal, outras informações sobre a imagem segmentada podem ser extraídas do histograma. Por exemplo, $p_{1}$ ($k^{*}$) e $p_{2}$ ($k^{*}$), indicam as probabilidades de cada classe conforme o limiar $k^*$. Da mesma forma, as médias $m_{1} (k^{*})$ e $m_{2} (k^{*})$ são estimativas da intensidade média das classes na imagem original.
%FIGURAS: RGB, EM NÍVEL DE CINZA, BINARIZADA E COM SEU THRESHOLD
\begin{figure}[tb]
\begin{center}
    \caption{Exemplo do método de Otsu e binarização} 

	\includegraphics[height=4cm]{images/mangaColor.png} \quad
	\includegraphics[height=4cm]{images/mangaGray.png} \quad 
	\includegraphics[height=4cm]{images/mangaBinaria.png}\quad
	\includegraphics[height=4.5cm]{images/mangaThreshold.png} \quad
...
\includegraphics[width=7cm, height=7cm]{images/mangaErodida.png}
...
\includegraphics[width=7cm, height=7cm]{images/mangaDilatada.png}
...
\includegraphics[width=7cm, height=7cm]{images/mangaAbertura.png}
...
\includegraphics[width=7cm, height=7cm]{images/mangaFechamento.png}
...
\begin{figure}[ht]
\centering
\caption{Exemplo de Transformada da Distância}
\includegraphics[width=7cm, height=7cm]{images/mangaTransDistancia.png}
...
No trabalho de \cite{dralle1996}  \sergio{foi feita a transformação da borda [COMO?]} da \sergio{subparcela [O QUE É ESSE TERMO?]} da imagem, em seguida foi aplicada a suavização do núcleo. 
 
A definição de \textit{suavização do núcleo \sergio{ACHO QUE NÃO É O TERMO CORRETO}} de uma imagem com um kernel gaussiano arbitrário, é determinado pelos três parâmetros $\sigma_{1}$, $\sigma_{2},$ e  $\rho$. Seja $x_{i} = (x_{i1},x_{i2})^{T}$ onde \textit{i=1,2, ..., I}, denotam os centros de pixels dos pixels \textit{I} de uma imagem. Coloque:
 \begin{equation}
...
\begin{center}
	...
	\includegraphics[height=4cm]{images/mangaBinaria.png}\quad
	\includegraphics[height=4cm]{images/mangaRotulada.png} \quad
