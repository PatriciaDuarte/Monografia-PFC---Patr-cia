...
No trabalho de \cite{dralle1996} a técnica de filtragem máxima local foi desenvolvida para aplicar no estudo de fotos pancromática aérea de \textit{Picea abies}. O número de árvores por hectares é estimado a partir do número de máximos acima de um determinado nível da imagem suavizada.\sergio{SUGIRO QUE NESSA SEÇÃO VC FALE SOMENTE DO MÉTODO.} \textcolor{green}{ATENÇÃO PROFESSOR: Neste artigo não encontrei algo exato que diz que kernel smoothing e filtragem maxima local são sinônimos.} \sergio{É A MESMA COISA. JÁ ACHEI ARTIGO QUE FALA ISSO.}
 
Primeiramente \sergio{foi feita a transformação da borda [COMO?]} da \sergio{subparcela [O QUE É ESSE TERMO?]} da imagem, em seguida foi aplicada a suavização do núcleo. 
