...
\sergio{FAZER EXEMPLO DE ABERTURA PARA ILUSTRAR}
...
O fechamento tende a suavizar seções de contornos, mas, ao contrário da abertura, geralmente funde quebras estreitas e golfos finos e longos, elimina pequenos orifícios e preenche lacunas no contorno. \sergio{FAZER EXEMPLO DE ABERTURA PARA ILUSTRAR}

\subsection{Rotulação de Componentes conectados}

Segundo \cite{Gonzales2010}, a capacidade de extrair componentes conectados de uma imagem binária é central para muitos aplicações automatizadas de análise de imagem. Seja $A$ um conjunto de pixels de objetos que consiste em um ou mais componentes conectados. \sergio{Seja $X_{0}$ um pixel pixel de objeto de $A$. Pode-se extrair o componente conectado pelo procedimento interativo dado por}: 
\begin{equation}
    X_{k} = (X_{k-1} \oplus B) \cap I \qquad \qquad k = 1,2,3...
\end{equation}
\sergio{onde $B$ é o elemento estruturante que define a conectividade}. O procedimento termina quando $X_{k} = X_{k-1}$. \sergio{Nesse momento pode-se extrair o componente de A ou rotular este. Este procedimento deve ser repetido para um novo pixel de objeto não rotulado, até que todos os componentes conectados sejam extraídos ou rotulados.} 

\subsection{Método de Otsu}

Segundo \cite{Gonzales2010}, o método de Otsu é ótimo no sentido de maximizar a variação entre classes, uma medida bem conhecida usada na análise estatística discriminante. \sergio{A ideia básica é separar os pixels em duas classes por um limiar apropriado, de a varianção de intensidade entre as classes seja máxima e a variação de intensidade dentro de cada classe seja mínima}. O método de Otsu tem a propriedade importante de que ele \sergio{pode} ser basear inteiramente no histograma de uma imagem \sergio{ $p_i$, que dá a probabilidade de ocorrência de cada nível de cinza $i$ da imagem.}
