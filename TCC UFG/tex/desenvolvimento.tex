...
No decorrer do desenvolvimento do PFC2, pretende-se coletar um maior número de imagens destas plantações e analisar plantações de árvores de outras espécies para a captura de imagens. Além disso, pretende-se criar imagens de \textit{ground-truth} para que se possa mensurar quantitativamente a qualidade dos algoritmos de detecção de árvores que serão desenvolvidos.
...
\begin{enumerate}
    ...
    \item Rotulação/contagem de árvores: Sobre o resultado da segmentação, os da filtragem máxima local aplicam um método de rotulação de componentes conectados para o caso de segmentação, e um simples cálculo de contagem dos valores máximos locais, no caso da aplicação do método de filtragem máxima local.
...
Para a conversão das imagens para níveis de cinza usamos a medida de luminância~\cite{Gonzales2010} que a projetada para casar a percepção humana de brilho, devido a proporção de células sensíveis a cada uma das cores primárias RGB. Assim, é usada combinação ponderada dos canais RGB:
